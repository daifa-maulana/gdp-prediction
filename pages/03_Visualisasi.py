...
with tab1:
    ...
    st.plotly_chart(plot_gdp_trend(df), use_container_width=True)
    st.info("""
    **Catatan:**
...
with tab2:
    ...
    corr = df.drop("Year", axis=1).corr()["GDP_Growth"].drop("GDP_Growth").sort_values(ascending=False)
...
with tab3:
    ...
    if mode == "Semua Sekaligus":
        st.plotly_chart(plot_all_scatter(df), use_container_width=True)
    else:
        features = ["Inflation", "Unemployment", "Population_Growth",
                    "Exports", "Imports", "FDI", "Exchange_Rate"]
        feat = st.selectbox("Pilih fitur:", features)
        st.plotly_chart(plot_scatter(df, feat), use_container_width=True)
